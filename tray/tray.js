...
function buildMenu(status, label) {
...
function createTray(status, label) {
  if (currentTray) {
    try {
  ...
  systray.onClick(action => {
    ...
    if (title.includes("关闭托盘")) {
      ...
      currentCallbacks.onSettings();
    } else if (title.includes("打开配置")) {
      currentCallbacks.onOpenConfig();
    } else if (title.includes("聚焦 opencode")) {
      focusOpenCode();
    } else if (title.includes("暂停通知") || title.includes("恢复通知")) {
      const config = require("./config.js").loadConfig();
      const newEnabled = !config.notification.enabled;
      currentCallbacks.onToggleNotify(newEnabled);
      // Update menu to reflect new toggle label
      systray.sendAction({
        type: "update-menu",
        menu: buildMenu(status, label),
...
function updateTray(status, label, onExit, onOpenConfig, onToggleNotify, onSettings, onUninstall) {
  currentCallbacks = { onExit, onOpenConfig, onToggleNotify, onSettings, onUninstall };
  if (!currentTray) {
    createTray(status, label);
    return;
  }
  try {
    currentTray.sendAction({
      type: "update-menu",
      menu: buildMenu(status, label),
      seq_id: 0,
    });
  } catch (e) {
    logger.error("Failed to update tray, recreating", { error: e.message });
    createTray(status, label);
  }
}
